...
const WeekView: React.FC<WeekViewProps> = ({
  ...
  // sideways has room for the full desktop labels.
  const { width } = useViewport();
  const isMobile = width < 640;

  // Horizontal swipe steps a week. The gesture is only claimed when it is
  // clearly horizontal and past a deliberate threshold, so it never steals a
  // vertical scroll through the campaign list or a tap on a bar.
  ...
  const handleTouchEnd = (e: React.TouchEvent) => {
    ...
    onAnchorDateChange(addDays(weekDays[0], dx < 0 ? 7 : -7));
  };
  ...
  return (
    ...
    >
      ...
      >
        <div className={`flex items-center gap-2 ${isMobile ? 'order-last self-center' : ''}`}>
          <button
        ...
        <div className={isMobile ? 'flex items-baseline justify-between gap-2 w-full' : 'contents'}>
          <h2 className={`font-bold text-white whitespace-nowrap ${isMobile ? 'text-sm' : 'text-lg'}`}>
            {formatWeekRange(weekDays[0], weekDays[6])}
            <span className="text-[#65B3AE] ml-2">{weekDays[6].getFullYear()}</span>
          </h2>

          <p className={`text-[#65B3AE] ${isMobile ? 'text-[10px] whitespace-nowrap' : 'text-xs'}`}>
            {isMobile
              ? `${weekCampaigns.length} active`
      ...
      {/* Day headers */}
      <div className={`flex-shrink-0 ${isMobile ? 'px-2 pt-2' : 'px-4 pt-3'}`}>
          ...
          {weekDays.map((day, index) => {
            ...
            return (
              <button
                ...
                className={`rounded-lg transition-all border ${isMobile ? 'py-1.5' : 'py-2'} ${
                  isToday
              ...
              >
                <span
                  className={`block font-bold uppercase tracking-wide text-[#7FD4D0] opacity-60 ${
                    isMobile ? 'text-[9px]' : 'text-[11px]'
                  }`}
                >
                  {isMobile ? DAY_NAMES[index].charAt(0) : DAY_NAMES[index]}
                </span>
                <span
                  className={`block font-bold ${isMobile ? 'text-base' : 'text-xl'} ${
                    isToday ? 'text-white' : 'text-[#7FD4D0]'
                  }`}
                >
                  {day.getDate()}
                </span>
                {/* "2 active" needs room seven times over; on a phone the
                    count alone carries it, and zero becomes a dim dot. */}
                <span
                  className={`inline-block font-bold ${
                    isMobile
                      ? 'mt-0.5 w-5 h-5 leading-5 rounded-full text-[10px]'
                      : 'mt-1 px-2 py-0.5 rounded-full text-[10px]'
                  } ${
                    count > 0
                ...
                >
                  {isMobile ? (count > 0 ? count : '·') : count > 0 ? `${count} active` : 'none'}
                </span>
              </button>
            );
          })}
        ...
        {weekCampaigns.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-center">
            ...
            <p className="text-xs text-[rgba(101,179,174,0.45)] mt-1">
              {isMobile
                ? 'Swipe left or right to step through weeks, or tap Today.'
          ...
          <div className="space-y-1.5">
            {weekCampaigns.map((campaign) => {
              ...
              return (
                <div key={campaign.id} className={`grid grid-cols-7 ${isMobile ? 'gap-0.5' : 'gap-1'}`}>
                  <button
                    ...
                    className={`rounded-lg text-left transition-all hover:brightness-125 overflow-hidden ${
